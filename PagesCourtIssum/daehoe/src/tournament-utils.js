...
export function compactRegionLabel(tournament = {}) {
  const region = tournament.regionSigungu || [tournament.regionSido, tournament.regionSigungu].filter(Boolean).join(" ") || tournament.regionSido;
  return String(region || "").replace(/^대한민국\s*/, "").replace(/^경기도\s*/, "").replace(/^서울특별시\s*/, "서울 ").trim() || "지역 미상";
}

export function divisionGroupValues(text = "") {
  ...
  return [...new Set(groups.length ? groups : ["기타 부서"])];
}
