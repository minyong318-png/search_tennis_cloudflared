...
-- Canonical snapshot used by the scheduled Daehoe crawl. Only the server-side
-- service role writes and reads these tables; the public app receives generated JSON.
create table if not exists daehoe_tournaments (
  id text primary key,
  source_type text not null,
  source_id text,
  start_date date,
...
create index if not exists daehoe_tournaments_active_date_idx
  on daehoe_tournaments(active, start_date, end_date);

create index if not exists daehoe_tournaments_source_idx
  on daehoe_tournaments(source_type, source_id);

create table if not exists daehoe_sync_state (
  key text primary key,
  value jsonb not null default '{}'::jsonb,
  updated_at timestamptz not null default now()
);

alter table daehoe_tournaments enable row level security;
alter table daehoe_sync_state enable row level security;
revoke all on table daehoe_tournaments from anon, authenticated;
revoke all on table daehoe_sync_state from anon, authenticated;
grant select, insert, update, delete on table daehoe_tournaments to service_role;
grant select, insert, update, delete on table daehoe_sync_state to service_role;
