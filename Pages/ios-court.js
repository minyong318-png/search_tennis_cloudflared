...
(function () {
  ...
  function renderPickerOptions() {
    const target = el("courtOptions"); if (!target) return; const search = (el("courtSearch")?.value || "").trim().toLocaleLowerCase(); target.replaceChildren();
    cityGroups().filter(group => !search || group.toLocaleLowerCase().includes(search)).forEach(group => {
  ...
  function bind() {
    el("courtFilterButton")?.addEventListener("click", event => openCourtPicker(event.currentTarget)); el("closeCourtPicker")?.addEventListener("click", closeCourtPicker); el("courtSearch")?.addEventListener("input", renderPickerOptions); el("resetFilters")?.addEventListener("click", resetFilters);
